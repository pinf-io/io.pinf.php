    ...
    describe("clean", function() {
        ...
        it("status", function(done) {
            return call(["status"], function(err, stdout) {
                ...
                if (ids) {
                    return done(new Error("PHP running at beginning of test!"));
                }
                return done();
            });
        });
        ...
        it("make request", function(done) {
            return REQUEST("http://127.0.0.1:" + config.php.httpd.port + "/index.php", function (err, res, body) {
                if (err) return done(err);
                ASSERT.equal(body, "Hello World from ROUTER");
                return REQUEST("http://127.0.0.1:" + config.php.httpd.port + "/index.html", function (err, res, body) {
                    if (err) return done(err);
                    ASSERT.equal(body, "Hello World from ROUTER");
                    return done();
                });
        ...
        it("status", function(done) {
            return call(["status"], function(err, stdout) {
                ...
                var ids = JSON.parse(stdout);
                if (ids) {
                    return done(new Error("PHP running at end of test!"));
                }
                return done();
            });
        });
